...
#### Project Context: SonarLint VS Code Extension (Internal Tool)
**Stack**: TypeScript 4.9.5, VS Code Extension API, LSP Client/Server, Node.js 20.12+, Mocha/Chai Testing
**Threat Profile**: Internal developer tool (calibrate security concerns accordingly)

#### 🔴 Critical (Blocks Deployment)
**Security vulnerabilities (calibrated for internal tool):**
- **HIGH PRIORITY**: Tokens/credentials stored in plain settings files (MUST use SecretStorage API)
- **HIGH PRIORITY**: LSP protocol message validation at client-server boundary
- Path traversal in file system operations (especially `FileSystemServiceImpl`)
- Command injection in Java VM args (`sonarlint.ls.vmargs`)
- Signature verification bypass in C/C++ analyzer loading (`cfamily/ondemand.ts`)
- Webview CSP headers missing or HTML not escaped (use `escapeHtml()` utility)
- User-provided paths not validated (Java home, Node path)

**Correctness Issues:**
- Logic errors that produce wrong results
- Missing error handling that causes crashes (all async ops need try-catch)
- Race conditions in LSP communication
- Data corruption risks
- Broken LSP protocol contracts
- Infinite loops or recursion
- Unhandled promise rejections (503+ async/await usages in codebase)

**Code Style Violations (REQUIRED):**
- Missing copyright header (LGPL v3 header required on ALL source files)
- `any` types used (flag for review - use `unknown` for truly dynamic types)
- Disposables not registered in `context.subscriptions` (memory leaks)
...
- Resource leaks (memory, file handles, connections, subscriptions)
- Missing timeout handling (especially for LSP requests)
- Inadequate logging for debugging (use `logToSonarLintOutput()` utility)
- Missing rollback/recovery logic
- TreeDataProvider refresh not properly implementing observer pattern

**Performance Issues:**
- Unnecessary language analyzer loads (should be on-demand)
- Unbounded memory growth
- Blocking I/O where async is expected
- Wholesale tree view refresh instead of selective updates

**Style Inconsistencies:**
- Naming violations: Services MUST use `*Service` suffix, TreeDataProviders MUST use `*TreeDataProvider` suffix
- Import order wrong (should be: node built-ins → external deps → internal modules)
- Constants not using SCREAMING_SNAKE_CASE
- camelCase not used for functions/vars, PascalCase not used for classes/types
- Prettier violations: 120 char width, single quotes, 2-space indent, no trailing commas
- Singleton pattern violated (services need static `init()` and `instance`)

**Pattern Violations:**
- Deviates from established project patterns
- Different error handling than rest of codebase
- Inconsistent state management (wrong VS Code API scope: application/resource/machine/window)

#### 🟢 Suggestion (Consider)
- Alternative approaches used elsewhere in codebase
- Documentation that might help future developers
- Test cases that might be worth adding (new features should have Mocha/Chai tests)
- Configuration that might need updating
- TypeScript type improvements (use union types for discriminated unions, `readonly` for immutable props)
- LSP protocol version sync with Java language server
- VS Code activation events optimization (currently uses `onStartupFinished`)
- Webpack bundle size optimization opportunities
