...
**Project-Specific Context**: For this TypeScript VS Code extension, ensure you capture:
- VS Code Extension API implementation details (commands, views, TreeDataProviders, webviews)
- LSP protocol changes (custom notifications, requests, client/server communication)
- Java language server modifications (startup, configuration, analyzer loading)
- Language analyzer integration work (JAR loading, signature verification)
- Connection management changes (SonarQube Server/Cloud, token handling via SecretStorage)
- Settings modifications (scope changes: application/resource/machine/window)
- Service pattern implementations (singleton pattern with static `init()` and `instance`)
- Test additions or modifications (Mocha/Chai suites, VS Code test environment)
- Webpack bundling changes (development vs production configuration)
- LGPLv3 copyright header compliance
