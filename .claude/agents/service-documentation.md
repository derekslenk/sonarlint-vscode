...
- JSDoc comments in TypeScript files (classes, interfaces, public methods)
- Copyright headers (REQUIRED: LGPLv3 header on ALL .ts source files)
- `package.json` contributions section (commands, views, settings, language model tools)
- Any other `.md` files that reference affected code

Gather the full list of documentation files that might be relevant.

**Project-Specific Context**: This is a TypeScript VS Code extension with:
- JSDoc for public APIs and interfaces
- LGPLv3 copyright header REQUIRED on all source files
- `package.json` as source of truth for extension contributions
- CLAUDE.md for architecture and development guidance
