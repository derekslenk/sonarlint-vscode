...
### Project-Specific Context: SonarLint VS Code Extension
**Stack**: TypeScript 4.9.5, VS Code Extension API, LSP Client/Server, Node.js 20.12+, Java Language Server, Mocha/Chai
**Key Areas to Research**:
- VS Code Extension API patterns (activation events, commands, views, webviews, TreeDataProviders)
- LSP client/server architecture (custom protocol methods, notifications, requests)
- Java language server lifecycle and communication (stdio, process spawning)
- Language analyzer JAR loading patterns (on-demand, signature verification)
- Connection management (SonarQube Server/Cloud, token storage via SecretStorage API)
- Settings scope hierarchy (application/resource/machine/window)
- Singleton service patterns (static `init()` and `instance`)
- Observer pattern for tree view updates (EventEmitter)
- Extension bundling (webpack, development vs production)
- Testing patterns (Mocha suites, VS Code test environment, async polling)

### Architecture Patterns
- Repository structure: super-repo, mono-repo, single-purpose, microservices
- Communication patterns: REST, GraphQL, gRPC, WebSockets, message queues, event buses, **LSP (Language Server Protocol)**
- State management: Redux, Context API, MobX, Vuex, Zustand, server state, **VS Code ExtensionContext (globalState, secrets)**
- Design patterns: MVC, MVVM, repository pattern, dependency injection, factory pattern, **singleton services**, **observer pattern**
