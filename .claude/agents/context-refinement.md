...
**Project-Specific Focus**: For this TypeScript VS Code extension, pay special attention to:
- LSP protocol message behaviors (custom notifications, request/response patterns)
- Java language server lifecycle surprises (startup, shutdown, analyzer loading)
- VS Code API quirks (activation timing, TreeDataProvider refresh patterns, webview lifecycle)
- SecretStorage API behaviors (token migration, persistence patterns)
- Language analyzer integration details (JAR loading, signature verification, on-demand patterns)
- Extension bundling issues (webpack, development vs production differences)
- Test environment constraints (VS Code test runner, Mocha timeout requirements)
- Connection management edge cases (SonarQube Server/Cloud authentication, SSL certificate handling)
